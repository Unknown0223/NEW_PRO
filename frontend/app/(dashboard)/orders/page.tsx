...
function ordersMutationFeedback(err: unknown, fallback: string): string {
...
function OrdersPageContent() {
  ...
  const expeditorSummaryRows = useMemo(() => {
    ...
    return Array.from(map.values()).sort((a, b) => b.orders - a.orders || a.expeditor.localeCompare(b.expeditor, "ru"));
  }, [rows]);
  ...
  function renderOrderDataCell(colId: string, o: OrderListRow): ReactNode {
    switch (colId) {
      ...
      case "warehouse_block_name":
        return o.warehouse_block_name?.trim() ? o.warehouse_block_name : "—";
      case "agent_name":
        return o.agent_name ?? "—";
      case "agent_code":
        return o.agent_code ?? "—";
              ...
              <div className="inline-flex rounded-md border border-border bg-background p-0.5">
                <button
                  ...
                  className={cn(
                    ...
                    ordersViewMode === "list" ? "bg-primary text-primary-foreground" : "text-muted-foreground"
                  )}
                ...
                <button
                  ...
                  className={cn(
                    "rounded px-2 py-1 text-xs",
                    ordersViewMode === "expeditor-summary" ? "bg-primary text-primary-foreground" : "text-muted-foreground"
                  )}
                    ...
                    <tbody>
                      {expeditorSummaryRows.map((r) => (
                        <tr key={r.expeditor} className="border-b border-border last:border-0">
                          ...
                          <td className="px-3 py-2 text-right tabular-nums">{formatGroupedInteger(r.orders)}</td>
                          ...
                          <td className="px-3 py-2 text-right tabular-nums">{formatGroupedInteger(r.delivering)}</td>
                          <td className="px-3 py-2 text-right tabular-nums">{formatGroupedInteger(r.delivered)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
