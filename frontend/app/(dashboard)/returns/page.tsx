...
function ReturnsPageContent() {
  ...
  const [returnStatus, setReturnStatus] = useState<"all" | "pending" | "posted" | "cancelled">("all");
  ...
  const listQ = useQuery({
    queryKey: ["returns", tenantSlug, limit, returnStatus],
    enabled: Boolean(tenantSlug) && hydrated && activeTab === "list",
    staleTime: STALE.list,
    queryFn: async () => {
      const qs = new URLSearchParams({ page: "1", limit: String(limit) });
      if (returnStatus !== "all") qs.set("status", returnStatus);
      const { data } = await api.get<{ data: ReturnRow[]; total: number }>(
        `/api/${tenantSlug}/returns?${qs.toString()}`
      );
      return data;
    }
  });

  const returnDetailQ = useQuery({
    queryKey: ["return-detail", tenantSlug, viewReturnId],
    enabled: Boolean(tenantSlug && viewReturnId != null),
    staleTime: STALE.detail,
    queryFn: async () => {
      const { data } = await api.get<ReturnDetail>(`/api/${tenantSlug}/returns/${viewReturnId}`);
      return data;
    }
  });
  ...
  return (
    <PageShell>
                ...
                <div className="flex items-center gap-3 px-3 pt-2">
                  ...
                  <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    ...
                    <select
                      value={returnStatus}
                      onChange={(e) =>
                        setReturnStatus(e.target.value as "all" | "pending" | "posted" | "cancelled")
                      }
      ...
      <Dialog open={viewReturnId != null} onOpenChange={(open) => !open && setViewReturnId(null)}>
        <DialogContent showCloseButton className="sm:max-w-3xl">
          <DialogTitle>
            {returnDetailQ.data?.number ?? "Возврат"}
          </DialogTitle>
          <DialogDescription>
            Детализация документа возврата
          </DialogDescription>
          {returnDetailQ.isLoading ? (
            <p className="text-sm text-muted-foreground">Загрузка…</p>
          ) : returnDetailQ.isError || !returnDetailQ.data ? (
            <p className="text-sm text-destructive">Деталь не загружена.</p>
          ) : (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>Склад: {returnDetailQ.data.warehouse_name}</div>
                <div>Статус: {returnDetailQ.data.status}</div>
                <div>Клиент: {returnDetailQ.data.client_name ?? "—"}</div>
                <div>Заказ: {returnDetailQ.data.order_number ?? "—"}</div>
              </div>
              <div className="overflow-x-auto rounded-lg border">
                <table className="w-full min-w-[640px] border-collapse text-xs">
                  ...
                  <tbody>
                    {returnDetailQ.data.lines.map((ln) => (
                      <tr key={ln.id} className="border-b border-border last:border-0">
                        <td className="px-3 py-2 font-mono">{ln.product_sku}</td>
                        <td className="px-3 py-2">{ln.product_name}</td>
                        <td className="px-3 py-2 text-right tabular-nums">{ln.qty}</td>
                        <td className="px-3 py-2 text-right tabular-nums">{ln.paid_qty ?? "—"}</td>
                        <td className="px-3 py-2 text-right tabular-nums">{ln.bonus_qty ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
