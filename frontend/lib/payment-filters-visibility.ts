...
/** Profilda kamroq hudud darajasi bo‘lsa, ortiqcha territoryN kalitlarini o‘chiradi (махалля yo‘q, max 4). */
export function clampPaymentFilterVisibilityToTerritoryLevels(
  ...
  const n = Math.max(0, Math.min(4, Math.floor(territoryLevelCount)));
  const next = { ...v };
  for (let i = 1; i <= 5; i++) {
    if (i > n) {
