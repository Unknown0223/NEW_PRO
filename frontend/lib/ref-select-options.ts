...
/** Ko‘p tanlov paneli ro‘yxati (id = saqlangan qiymat). */
export type PickerListItem = { id: string; title: string; searchText?: string | null };
...
/** Ko‘p tanlov paneli uchun: `id` = saqlangan `value` (takrorlanmasin). */
export function refOptionsToPickerItems(options: RefSelectOption[]): PickerListItem[] {
  ...
  for (const o of options) {
    ...
    out.push({
      id: value,
      title: label,
      searchText: label !== value ? label : undefined
    });
  }
