...
function formatCellValue(
  ...
  return {
    value: s,
    rawValue: null,
    formatted: formatPivotMemberLabel(field?.id, s),
    isEmpty: false
  };
}

/**
 * WDR «Плоская» forma: agregatsiyasiz raw qatorlar, har maydon alohida ustun.
 * Frontendda majburiy — worker/eski keshdan mustaqil.
 */
export function buildFlatPivotData(
  ...
  const showCurrency = collectCurrencyCodesFromPivot(config, enrichedFields).length > 1;
  ...
  if (config.options.showGrandTotal && workingData.length > 0) {
    const cells: PivotCell[] = columnIds.map((id, colIdx) => {
      ...
      if (!isNumeric) {
        return { columnKey: id, value: null, rawValue: null, formatted: "", isEmpty: true };
      }
      let sum = 0;
...
/**
 * Flat jadval — katta datasetda UI bloklanmasligi uchun batch + yield.
 */
export async function buildFlatPivotDataAsync(
  rawData: Record<string, unknown>[],
  fields: PivotField[],
  config: PivotConfig,
  startTime = performance.now()
  ...
  for (let index = 0; index < workingData.length; index++) {
    ...
    rows.push({ key: `flat-${index}`, depth: 0, cells });
    if (index > 0 && index % FLAT_BUILD_CHUNK === 0) {
      await yieldToMain();
    }
  }
