...
export function BonusStrategySettings() {
  ...
  const saveMut = useMutation({
    ...
    onError: (e: unknown) => {
      if (e instanceof Error && e.message === "badmax") {
      ...
      setMsg(getUserFacingError(e, "Ошибка"));
    }
  });
  ...
  return (
    <>
      {!hydrated ? (
        ...
        <p className="text-sm text-destructive">{getUserFacingError(loadError, "Не удалось загрузить.")}</p>
      ) : (
