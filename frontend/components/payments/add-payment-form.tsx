...
type AllocationMode = "cash" | "consignment" | "none";

type OpenAllocationOrder = {
  order_id: number;
  order_number: string;
  created_at: string;
...
export function AddPaymentForm({
  ...
  const [allocationMode, setAllocationMode] = useState<AllocationMode>("none");
  ...
  const orderAllocQ = useQuery({
    ...
    enabled:
      ...
      (allocationMode === "cash" || allocationMode === "consignment"),
    staleTime: STALE.detail,
    queryFn: async () => {
      const params = new URLSearchParams({ client_id: clientId.trim(), mode: allocationMode });
      if (agentFilterId.trim()) params.set("agent_id", agentFilterId.trim());
      const { data } = await api.get<{ data: OpenAllocationOrder[] }>(
        `/api/${tenantSlug}/payments/open-orders?${params.toString()}`
      );
      return data.data;
    }
  });
  ...
  useEffect(() => {
    setSelectedOrderIds([]);
  }, [allocationMode, clientId, agentFilterId]);
  ...
  const submitMut = useMutation({
    mutationFn: async () => {
      ...
      for (const b of blocks) {
        ...
        if (!pt) throw new Error("payment_type");
        ...
        payloads.push({
          ...
          paid_at,
          allocation_mode: allocationMode,
          ...(selectedOrderIds.length > 0 ? { allocation_order_ids: selectedOrderIds } : {}),
          ...(ledger_agent_id_locked != null ? { ledger_agent_id: ledger_agent_id_locked } : {}),
          ...(agentFilterId.trim()
            ? { allocation_agent_id: Number.parseInt(agentFilterId.trim(), 10) }
            : {})
        });
      }
    ...
    onError: (e: unknown) => {
      ...
      if (e instanceof Error && e.message === "payment_type") {
        setFormErr("Выберите способ оплаты.");
        return;
      }
        ...
        if (flat) {
          ...
          setFormErr(
            hint
              ...
              : withApiSupportLine(getUserFacingError(e, "Не удалось сохранить платёж."), e)
          );
          return;
        }
      }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 rounded-lg border border-border/70 bg-muted/20 p-3">
          <Label htmlFor="add-pay-allocation-mode">Режим закрытия заказов</Label>
          <FilterSelect
              ...
              <table className="w-full text-xs">
                <thead className="app-table-thead">
                  <tr>
                    ...
                    <th className="px-2 py-1.5 text-right">Долг</th>
                  </tr>
                </thead>
                <tbody>
                  {(orderAllocQ.data ?? []).length === 0 ? (
                    <tr>
                    ...
                    (orderAllocQ.data ?? []).map((o) => {
                      const checked = selectedOrderIds.includes(o.order_id);
                      const due = (o.consignment_due_date ?? o.created_at).slice(0, 10);
                      return (
                        <tr key={o.order_id} className="border-t">
                          ...
                          <td className="px-2 py-1.5">{due}</td>
                          <td className="px-2 py-1.5 text-right tabular-nums">{o.outstanding}</td>
                        </tr>
                      );
                    })
                  )}
