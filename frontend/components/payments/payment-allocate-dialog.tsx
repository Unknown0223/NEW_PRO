...
function paymentAllocUiError(err: unknown, fallback: string): string {
  if (isAxiosError(err)) {
...
export function PaymentAllocateDialog({ open, onOpenChange, tenantSlug, payment, onAllocated }: Props) {
  ...
  const errorMsg = allocQ.isError
    ? paymentAllocUiError(allocQ.error, "Не удалось загрузить распределения")
    : allocateMut.isError
      ? paymentAllocUiError(allocateMut.error, "Распределение не выполнено")
      : null;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg" data-testid="payment-allocate-dialog">
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          >
            {allocateMut.isPending ? "Распределение…" : "Распределить"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
