...
function parseLineIndexFieldErrors(per: Record<string, string>): Record<number, string> {
  ...
  for (const [idxStr, msgs] of Object.entries(acc)) {
    out[Number.parseInt(idxStr, 10)] = msgs.join(" · ");
  }
  return out;
}

function validationErrorBanner(e: unknown, fallback: string): string {
  if (isAxiosError(e)) {
    ...
    if (flat) {
      ...
      return line
        ...
        : withApiSupportLine(getUserFacingError(e, fallback), e);
    }
  }
...
export function InventoryTakeEditor({ tenantSlug, takeId, onClose }: Props) {
  ...
  const [lineFieldErrs, setLineFieldErrs] = useState<Record<number, string>>({});
  ...
  const saveLinesMut = useMutation({
    ...
    onError: (e) => {
      if (isAxiosError(e)) {
        const flat = getZodFlattenFromApiErrorBody(e.response?.data);
        if (flat) {
          setLineFieldErrs(parseLineIndexFieldErrors(firstMessagePerField(flat)));
          setErrorMsg(validationErrorBanner(e, "Строки не сохранены"));
          return;
        }
      }
      setLineFieldErrs({});
      setErrorMsg(getUserFacingError(e, "Строки не сохранены"));
    }
  });
  ...
  const postMut = useMutation({
    ...
    onError: (e) => {
      if (isAxiosError(e)) {
        ...
        if (flat) {
          setLineFieldErrs({});
          setErrorMsg(validationErrorBanner(e, "Проведение не выполнено"));
          return;
        }
      }
      setLineFieldErrs({});
  ...
  const cancelMut = useMutation({
    ...
    onError: (e) => {
      if (isAxiosError(e)) {
        ...
        if (flat) {
          setLineFieldErrs({});
          setErrorMsg(validationErrorBanner(e, "Отмена не выполнена"));
          return;
        }
      }
      setLineFieldErrs({});
      setErrorMsg(getUserFacingError(e, "Отмена не выполнена"));
    }
  });
              ...
              <tbody>
                {lineRows.length === 0 ? (
                  ...
                  lineRows.map((r, idx) => {
                    ...
                    const lineHint = lineFieldErrs[idx];
                    return (
                      <tr key={r.product_id} className="border-t">
                        ...
                        <td className="px-3 py-2 text-right">
                          ...
                          />
                          {lineHint ? (
                            <p className="mt-1 max-w-[10rem] text-[11px] leading-snug text-destructive" role="alert">
                              {lineHint}
                            </p>
                          ) : null}
                        </td>
