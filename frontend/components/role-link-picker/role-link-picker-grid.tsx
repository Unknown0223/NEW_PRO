...
export function RoleLinkPickerGrid({
  ...
  const visibleColumns = columns
    .map((col) => {
      ...
      return { col, users, allIds, setForRole, allOn };
    })
      ...
      >
        ...
        >
          {visibleColumns.length === 0 ? (
            <div className="flex min-h-[120px] w-full items-center justify-center rounded-md border border-dashed border-border/70 bg-background/70 px-3 py-6 text-center text-xs text-muted-foreground sm:text-sm">
              {hasActiveSearch ? "По вашему поиску совпадений нет" : "Нет данных для отображения"}
            </div>
          ) : (
            visibleColumns.map(({ col, users, allIds, setForRole, allOn }) => (
              <div
                key={col.role}
                className={cn(
