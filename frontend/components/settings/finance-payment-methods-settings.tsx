...
export function FinancePaymentMethodsSettings() {
  ...
  useEffect(() => {
    const flag = profileQ.data?.feature_flags?.show_order_payment_method_selector;
    if (typeof flag === "boolean") {
      setShowOrderPaymentMethodSelector(flag);
    }
  }, [profileQ.data?.feature_flags]);
        ...
        <div className="rounded-lg border bg-card p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <label className="inline-flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={showOrderPaymentMethodSelector}
                onChange={(e) => {
                  const next = e.target.checked;
                  setShowOrderPaymentMethodSelector(next);
                  saveOrderCreateFlagMut.mutate(next);
                }}
        ...
        <DialogContent className="sm:max-w-[520px]" showCloseButton>
          ...
          <div className="grid gap-3">
            ...
            <div className="grid gap-1.5">
              ...
              {pickZodLeaf(serverFieldErrs, "currency_code") ? (
                <p className="text-xs text-destructive">{pickZodLeaf(serverFieldErrs, "currency_code")}</p>
              ) : null}
            </div>
            <div className="grid gap-1.5">
            ...
            <div className="grid gap-1.5">
              ...
              {pickZodLeaf(serverFieldErrs, "color") ? (
                <p className="text-xs text-destructive">{pickZodLeaf(serverFieldErrs, "color")}</p>
              ) : null}
            </div>
