...
export function ClientProfileEquipmentTab({ tenantSlug, clientId }: { tenantSlug: string; clientId: number }) {
  ...
  const productsQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get<{ data: ProductOption[] }>(`/api/${tenantSlug}/products?${params}`);
      return data.data ?? [];
    }
  });
  ...
  const selectedProduct = useMemo(() => {
    ...
    return (productsQ.data ?? []).find((p) => p.id === id) ?? null;
  }, [productId, productsQ.data]);

  const createM = useMutation({
    mutationFn: async () => {
      const finalInventoryType = selectedProduct?.name?.trim() || inventoryType.trim();
      const finalEquipmentKind = selectedProduct?.sku?.trim() || equipmentKind.trim();
      const { data } = await api.post<ClientEquipmentRow>(`/api/${tenantSlug}/clients/${clientId}/equipment`, {
        inventory_type: finalInventoryType,
        equipment_kind: finalEquipmentKind || null,
        ...
        <Card className="border border-border/90 shadow-panel">
          <CardContent className="grid gap-3 p-3 sm:grid-cols-2 sm:p-4">
            <div className="space-y-1 sm:col-span-2">
              <Label className="text-xs">Оборудование товар *</Label>
              <select
            ...
            <div className="space-y-1 sm:col-span-2">
              ...
              <Input
                className="h-9"
                value={selectedProduct?.name ?? inventoryType}
            ...
            <div className="space-y-1">
              <Label className="text-xs">Код / SKU</Label>
              <Input
                className="h-9"
                value={selectedProduct?.sku ?? equipmentKind}
                onChange={(e) => setEquipmentKind(e.target.value)}
                readOnly={Boolean(selectedProduct)}
              />
            </div>
            <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-2 sm:col-span-2">
              <button
                ...
                disabled={(!(selectedProduct?.name?.trim() || inventoryType.trim())) || createM.isPending}
            ...
            <table className="w-full min-w-[640px] border-collapse text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/50 text-left text-xs text-muted-foreground">
                  ...
                  <th className="px-3 py-2">Код / SKU</th>
