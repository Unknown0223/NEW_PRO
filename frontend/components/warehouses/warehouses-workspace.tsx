...
function warehouseMutationError(err: unknown): string {
  ...
  if (code === "WarehouseNameExists") return withApiSupportLine("Склад с таким названием уже есть.", err);
  if (code === "WarehouseHasStock") return withApiSupportLine("На складе есть остатки — удаление невозможно.", err);
  if (code === "WarehouseHasOrders") return withApiSupportLine("Есть заказы по этому складу — удаление невозможно.", err);
  if (status === 400) return withApiSupportLine("Проверьте данные (название обязательно).", err);
  if (status === 404) return withApiSupportLine("Склад не найден.", err);
  if (code === "UserRoleMismatch" || code === "InvalidLinkRole") {
    return withApiSupportLine("Роль пользователя не соответствует выбранному столбцу.", err);
  }
  if (code === "UserNotFound") return withApiSupportLine("Пользователь не найден.", err);
...
export function WarehousesWorkspace({ tenantSlug, canWrite }: Props) {
  ...
  const listQ = useQuery({
    queryKey: ["warehouses-table", tenantSlug, tab, page, limit, search],
  ...
  const rawRows = useMemo(() => listQ.data?.data ?? [], [listQ.data?.data]);
  const rows = useMemo(() => {
    const out = [...rawRows];
    out.sort((a, b) => {
