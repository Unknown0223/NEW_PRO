...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-9 w-full items-center justify-between gap-2 whitespace-nowrap rounded-lg border border-input bg-transparent px-2.5 py-1 text-sm shadow-sm outline-none transition-colors",
      "placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50",
      "disabled:cursor-not-allowed disabled:opacity-50 dark:bg-input/30 [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
  ...
  <SelectPrimitive.Portal>
    ...
    >
      <SelectPrimitive.Viewport
        className={cn(
          "max-h-[min(280px,var(--radix-select-content-available-height))] overflow-y-auto p-1",
