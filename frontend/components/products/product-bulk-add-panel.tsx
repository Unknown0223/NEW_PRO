...
function itemsZodRowMessages(per: Record<string, string>): Record<number, string> {
...
export function ProductBulkAddPanel({
  ...
  const [rowZodErrs, setRowZodErrs] = useState<Record<number, string>>({});
  ...
  const saveMut = useMutation({
    ...
    onError: (e: unknown) => {
      ...
      if (isAxiosError(e)) {
        ...
        if (flat) {
          setRowZodErrs(itemsZodRowMessages(firstMessagePerField(flat)));
      ...
      setRowZodErrs({});
      setSaveErr(getUserFacingError(e, "Xato"));
    }
  });
              ...
              {rows.map((r, i) => (
                <tr key={i} className="border-t">
                  ...
                  <td className="px-1 py-2 align-middle">
                    ...
                    {rowZodErrs[i] ? (
                      <p
                        className="mt-0.5 max-w-[14rem] text-[10px] leading-snug text-destructive"
                        role="alert"
                      >
                        {rowZodErrs[i]}
                      </p>
