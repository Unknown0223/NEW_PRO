...
export const OrdersPage: React.FC = () => {
  ...
  const handleRowAction = useCallback((orderId: string, action: string) => {
    if (action === 'return_from_shelf') {
      alert(`Возврат с полки по заказу ${orderId}`);
    } else if (action === 'change_delivery_date') {
      alert(`Изменить дату доставки для ${orderId}`);
    } else if (action === 'change_ship_date') {
      alert(`Изменить ожидаемую дату отгрузки для ${orderId}`);
    }
  }, []);

  const handleBulkDelete = useCallback(() => {
    if (confirm(`Удалить ${selectedRows.size} заказов?`)) {
