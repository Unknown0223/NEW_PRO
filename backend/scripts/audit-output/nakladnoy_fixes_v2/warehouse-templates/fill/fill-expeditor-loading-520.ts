...
function writeGroupRow(
  ...
  // TUZATISH: sum — son formatda (matn emas), 3 xonali
  if (sum > 0) {
    const sCell = sheet.getCell(row, COL.sum);
    sCell.value = sum;
...
function writeProductRow(
  ...
  // TUZATISH: price va sum — son formatda, 3 xonali
  if (ln.price > 0) {
    const pCell = sheet.getCell(row, COL.price);
...
export function fillExpeditorLoading520(
  ...
  // TUZATISH: jami summa — son formatda
  if (grandSum > 0) {
    const tsCell = sheet.getCell(totalRowFinal, COL.price);
