...
export async function processBackgroundJob(job: Job): Promise<unknown> {
  ...
  if (job.name === "generate_pdf") {
    const d = job.data as { tenant_id?: number; order_id?: number };
    return { ok: true, kind: "generate_pdf", tenant_id: d.tenant_id ?? null, order_id: d.order_id ?? null };
  }

  if (job.name === "cleanup_old_logs") {
