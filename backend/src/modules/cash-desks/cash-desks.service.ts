...
const CASH_DESK_SINGLE_USER_ROLES = ["cashier", "collector"] as const;

/** Bitta kassa — bir vaqtda bitta inkasator (cashier/collector link_role). */
export async function assertCashDeskCollectorNotOccupied(
  cashDeskId: number,
  userId: number,
  linkRole: string
): Promise<void> {
  if (!CASH_DESK_SINGLE_USER_ROLES.includes(linkRole as (typeof CASH_DESK_SINGLE_USER_ROLES)[number])) {
    return;
  }
  const other = await prisma.cashDeskUserLink.findFirst({
    where: {
      ...
      user_id: { not: userId },
      link_role: { in: [...CASH_DESK_SINGLE_USER_ROLES] }
    },
    select: { user_id: true }
  });
...
export async function createCashDeskUserLink(
  ...
  await assertCashDeskCollectorNotOccupied(cashDeskId, userId, linkRole);
  try {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (body.links) {
      ...
      for (const l of body.links) {
        await assertCashDeskCollectorNotOccupied(id, l.user_id, l.link_role);
      }
