...
export async function patchUserUiPreferences(
  ...
  const currentJson = JSON.stringify(asRecord(row.ui_preferences));
  const currentLen = currentJson.length;
  const merged = mergeUiPreferences(row.ui_preferences, patch);
  const json = JSON.stringify(merged);
  const nextLen = json.length;
  const max = MAX_UI_PREFS_JSON_CHARS;

  // Legacy rows can already be oversized; allow non-growing updates so users
  // can still change preferences without increasing storage pressure.
  if ((currentLen <= max && nextLen > max) || (currentLen > max && nextLen > currentLen)) {
