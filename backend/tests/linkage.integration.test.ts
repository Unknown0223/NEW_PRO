...
describe.skipIf(!dbReady)("linkage constraint scope (database)", () => {
  ...
  it("selected_agent_id bo‘yicha options endpointlari bir xil scope qaytaradi", async () => {
    ...
    const agents = (agentRes.body?.data ?? []) as Array<{ id: number; login: string }>;
    if (agents.length === 0) return;
    const picked = agents.find((a) => a.login === "agent") ?? agents[0]!;
    const selectedAgentId = picked.id;
    ...
    if (scope.expeditor_ids.length > 0) {
