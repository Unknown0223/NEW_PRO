...
function matchRule(cell: PivotCell, rule: ConditionalFormatRule): boolean {
  if (cell.columnKey === "__row_label__" || cell.isEmpty) return false;
  if (rule.fieldId) {
    const matchesField =
      cell.columnKey === rule.fieldId ||
...
export function getConditionalFormatStyle(
  ...
  for (const rule of rules) {
    if (!matchRule(cell, rule)) continue;
    return {
